...
function prepareOrderData() {
    ...
    $(document).ready(function() {
        $('#addOrderForm').off('submit'); // Remove existing handler
        
        $('#addOrderForm').on('submit', function(e) {
            e.preventDefault();
            
            if (selectedProducts.length === 0) {
                showToast('Please add products to your order', 'error');
                return;
            }
            ...
            if (poNumber && username) {
                showToast(`Creating order: ${poNumber} for ${username}...`, 'info');
            }
    ...
    // Function to fetch client shipping information with improved UX
    function fetchClientInfo() {
        const username = document.getElementById('username').value;
        if (!username) {
            resetClientInfoDisplays();
            return;
        }
        
        // Show loading state
        ['bill_to_display', 'bill_to_attn_display', 'ship_to_display', 'ship_to_attn_display'].forEach(id => {
        ...
        document.getElementById('edit_client_button').disabled = false;
        
        // Make AJAX request to get client info
        $.ajax({
            url: '/backend/get_client_info.php',
            type: 'POST',
            data: { username: username },
            dataType: 'json',
            success: function(response) {
                if (response.success) {
                    // Update display fields with improved formatting
                    updateInfoField('bill_to_display', response.bill_to || response.company_address);
                    updateInfoField('bill_to_attn_display', response.bill_to_attn);
                    updateInfoField('ship_to_display', response.ship_to || response.bill_to || response.company_address);
                    updateInfoField('ship_to_attn_display', response.ship_to_attn);
                    
                    // Check for completeness of address information
                    const hasShippingAddress = response.ship_to || response.bill_to || response.company_address;
                    
                    // Show warnings if needed
                    const warningDiv = document.querySelector('.info-warning');
                    if (warningDiv) {
                        warningDiv.remove();
                    }
                    ...
                    if (!hasShippingAddress) {
                        ...
                        warning.innerHTML = '<i class="fas fa-exclamation-triangle"></i> This client has no shipping address in their profile. Please update the client information.';
                        
                        document.querySelector('.shipping-info-container').appendChild(warning);
                    }
                    
                    // Add status indicators
                    addAddressStatusIndicators(response);
                } else {
                    resetClientInfoDisplays('Error loading data');
                    showToast('Failed to load client information: ' + response.message, 'error');
                }
            },
            error: function() {
                resetClientInfoDisplays('Server connection error');
                showToast('Failed to connect to server. Please try again.', 'error');
            }
        });
    ...
    // Helper function to update info fields with proper display
    function updateInfoField(elementId, value) {
        const element = document.getElementById(elementId);
        
        if (value && value.trim() !== '') {
            element.textContent = value;
            element.className = 'info-field has-data';
        } else {
            element.innerHTML = '<span class="not-provided">Not provided</span>';
            element.className = 'info-field';
        }
    }

    // Reset all client info displays
    function resetClientInfoDisplays(message = null) {
        ['bill_to_display', 'bill_to_attn_display', 'ship_to_display', 'ship_to_attn_display'].forEach(id => {
            const element = document.getElementById(id);
            if (message) {
                element.innerHTML = `<span class="placeholder">${message}</span>`;
            } else {
                element.innerHTML = '<span class="placeholder">Select a client to view information</span>';
            }
            element.className = 'info-field';
        });
        
        // Disable the edit button
        document.getElementById('edit_client_button').disabled = true;
    ...
    // Add address status indicators
    function addAddressStatusIndicators(response) {
        const hasBillingInfo = response.bill_to || response.company_address;
        const hasBillingAttn = response.bill_to_attn;
        const hasShippingInfo = response.ship_to;
        const hasShippingAttn = response.ship_to_attn;
        
        // Get the column headers
        const billingHeader = document.querySelector('.shipping-info-column:first-child h4');
        const shippingHeader = document.querySelector('.shipping-info-column:last-child h4');
        ...
        if (hasBillingInfo) {
            billingHeader.insertAdjacentHTML('beforeend', 
                `<span class="status-badge ${hasBillingAttn ? 'status-complete' : 'status-partial'}">
                    ${hasBillingAttn ? 'Complete' : 'Partial'}
                 </span>`);
        } else {
            billingHeader.insertAdjacentHTML('beforeend', 
        ...
        // Add shipping status indicator
        if (hasShippingInfo) {
            shippingHeader.insertAdjacentHTML('beforeend', 
                `<span class="status-badge ${hasShippingAttn ? 'status-complete' : 'status-partial'}">
                    ${hasShippingAttn ? 'Complete' : 'Partial'}
                 </span>`);
        } else if (hasBillingInfo) {
            // Using billing info as fallback
            shippingHeader.insertAdjacentHTML('beforeend', 
                '<span class="status-badge status-partial">Using Billing</span>');
        } else {
            shippingHeader.insertAdjacentHTML('beforeend', 
                '<span class="status-badge status-incomplete">Missing</span>');
        }
    ...
    // Function to edit client address
    function editClientAddress() {
        ...
        // Redirect to client account page with username
        window.location.href = `/public/pages/accounts_clients.php?edit=${username}&returnTo=pending_orders.php`;
    }
    ...
    function saveFormState() {
        const formState = {
            ...
            // Save cart as well
            cart: localStorage.getItem('cart') || JSON.stringify(selectedProducts)
        };
        
        localStorage.setItem('pendingOrderFormState', JSON.stringify(formState));
    }
    ...
    $(document).ready(function() {
        ...
        $("#delivery_date").datepicker({
            ...
            beforeShowDay: function(date) {
                ...
                return [day === 1 || day === 3 || day === 5]; // Only allow Monday, Wednesday, Friday
            }
        });
        ...
        // Check if returning from client edit
        const urlParams = new URLSearchParams(window.location.search);
        if (urlParams.get('from') === 'client_edit') {
            // Restore form state
            const formState = JSON.parse(localStorage.getItem('pendingOrderFormState') || '{}');
            ...
            if (formState.delivery_date) {
                $('#delivery_date').val(formState.delivery_date);
            }
            
            if (formState.special_instructions) {
                $('#special_instructions').val(formState.special_instructions);
            }
            
            if (formState.cart) {
                try {
                    selectedProducts = JSON.parse(formState.cart);
                    updateOrderSummary();
                } catch (e) {
            ...
            // Clear stored state
            localStorage.removeItem('pendingOrderFormState');
            
            // Show success message
            showToast('Client information has been updated successfully!', 'success');
        }
    });
    </script> 
</body>
