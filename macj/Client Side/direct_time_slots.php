<?php
// Ultra-simplified version with maximum error reporting
...
error_log("DIRECT_TIME_SLOTS.PHP ACCESSED");
...
if (!isset($_SESSION['role']) || $_SESSION['role'] !== 'client') {
    error_log("User not logged in or not a client. Redirecting to SignIn.php");
    header("Location: SignIn.php");
    exit;
}

// Get client info - simplified approach
$client_id = $_SESSION['client_id'] ?? 0;
$client_name = $_SESSION['fullname'] ?? 'Client';
error_log("Client ID: {$client_id}, Name: {$client_name}");
...
try {
    ...
    if (!$client) {
        ...
        $client = [
            ...
            'email' => '',
            'contact_number' => '',
            'type_of_place' => 'Not specified',
            'location_address' => 'Not specified'
        ];
    } else {
        error_log("Client data retrieved successfully");
    }
} catch (Exception $e) {
...
try {
    ...
    error_log("Found " . count($bookedTimes) . " booked times for date {$selectedDate}");
    if (!empty($bookedTimes)) {
        error_log("Booked times: " . implode(", ", $bookedTimes));
    }
} catch (Exception $e) {
    error_log("Error getting booked times: " . $e->getMessage());
}
?>
<!DOCTYPE html>
<html lang="en">
...
<body>
    <div class="container">
        ...
        <div class="alert alert-info">
            ...
            <p>Follow these steps to schedule your appointment:</p>
            <ol>
        ...
        <div class="date-selector">
            <form method="get" action="direct_time_slots.php">
        ...
        <div class="time-slots-container">
            ...
            <p><strong>Instructions:</strong> Click on an available time slot below to schedule your appointment. Booked slots are shown in red and cannot be selected.</p>
            ...
            <div class="time-slots">
                ...
                if ($isToday) {
                    ...
                    error_log("Buffer time: " . $bufferTime->format('H:i:s'));
                }
                    ...
                    for ($minute = 0; $minute < 60; $minute += 60) {
                        ...
                        if ($isToday) {
                            ...
                            if ($slotTime <= $bufferTime) {
                                $isPastTime = true;
                                error_log("Time slot $time is in the past or too soon");
                            }
                        }
    ...
    <script>
        // Ultra-simple JavaScript with no dependencies
        document.addEventListener('DOMContentLoaded', function() {
            console.log('DOM fully loaded');

            // Handle time slot selection
            var timeSlots = document.querySelectorAll('.time-slot:not(.booked):not(.past-time)');
            ...
            for (var i = 0; i < timeSlots.length; i++) {
                timeSlots[i].addEventListener('click', function() {
                    ...
                    var displayTime = this.textContent;
                    console.log('Selected time:', selectedTime, 'Display time:', displayTime);

                    // Update form values
                    document.getElementById('preferred_time').value = selectedTime;
            ...
            document.getElementById('bookingForm').addEventListener('submit', function(e) {
                ...
                console.log('Form submitted, preferred time:', preferredTime);
            ...
            if (typeof jQuery !== 'undefined') {
                console.log('jQuery is available, adding additional handlers');

                // Handle time slot selection with jQuery
                $(document).on('click', '.time-slot:not(.booked):not(.past-time)', function() {
                    $('.time-slot').removeClass('selected');
                    $(this).addClass('selected');

                    var selectedTime = $(this).data('time');
                    var displayTime = $(this).text();
