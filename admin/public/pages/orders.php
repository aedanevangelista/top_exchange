...
// Further Modifications: Added Order Type (Online/Walk-In) and Company display/input logic

session_start();
include "../../backend/db_connection.php"; // Make sure this path is correct
include "../../backend/check_role.php"; // Make sure this path is correct
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_delivery_date']) && isset($_POST['po_number']) && isset($_POST['new_delivery_date'])) {
    ...
    if ($is_valid_day_update && $is_valid_days_gap_update) {
        ...
        if ($stmt_update_date->execute()) {
            ...
            if ($user_data_email_update && !empty($user_data_email_update['email'])) {
                ...
                $headers_update = "From: no-reply@topexchange.com"; // Replace with your actual sender email
    ...
    header("Location: orders.php" . (isset($_GET['sort']) ? "?sort=".$_GET['sort']."&direction=".$_GET['direction'] : "")); // Preserve sort
    exit();
}

// Fetch active clients for the dropdown
$clients_list = []; // Renamed to avoid conflict
$clients_data_map = []; // To store all client data by username for JS
...
while ($row_client = $result_clients->fetch_assoc()) {
    $clients_list[] = $row_client['username']; // For dropdown options
    $clients_data_map[$row_client['username']] = [ // For JS access
...
// Fetch orders
$orders_list = []; // Renamed
// Added o.order_type, o.company to SELECT
$sql_orders = "SELECT o.id, o.po_number, o.order_type, o.username, o.company, o.order_date, o.delivery_date, o.delivery_address, o.orders, o.total_amount, o.status, o.progress,
        ...
        OR (o.status = 'For Delivery' AND o.progress < 100) "; // Show For Delivery if not 100% for some reason

$orderByClause = 'o.' . $sort_column; // Already validated $sort_column
...
// Helper function to get next available delivery date (Mon, Wed, Fri), at least 5 days from a base date
...
function isValidDeliveryGap($orderDate_str, $deliveryDate_str, $minDays = 5) {
    ...
    try {
        ...
        // Delivery date must be on or after order date + minDays
...
<head>
    ...
    <style>
        /* --- Styles from commit e801e50... and subsequent additions --- */
        ...
        .summary-table th:nth-child(6), .summary-table td:nth-child(6) { width: 10%; text-align: center; } /* Remove button */
        ...
        .po-table td:nth-child(4), .po-table td:nth-child(5), .po-table td:nth-child(6) { text-align: right;} /* Align numeric columns right */
        ...
        #pdfPreview { display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background-color: rgba(0, 0, 0, 0.7); z-index: 1070; overflow: auto; } /* Higher z-index */
        ...
        .instructions-modal-content { background-color: #fff; margin: 10% auto; padding: 0; border-radius: 8px; width: 90%; max-width: 600px; position: relative; box-shadow: 0 5px 15px rgba(0,0,0,0.3); animation: modalFadeIn 0.3s ease-out; max-height: 80vh; display: flex; flex-direction: column; }
        ...
        .btn-info { font-size: 0.75em; opacity: 0.8; margin-top: 3px; display: block; } /* For modal status buttons */
        /* Raw Materials Table */
        ...
        /* Progress Bar for main table */
        .active-progress { background-color: #d1e7ff; color: #084298; } .pending-progress, .rejected-progress { background-color: #f8d7da; color: #721c24; } /* For non-active orders */
        ...
        /* Special Instructions Textarea in Modal */
        ...
        /* Confirmation Modals */
        .confirmation-modal { display: none; position: fixed; z-index: 1100; left: 0; top: 0; width: 100%; height: 100%; background-color: rgba(0,0,0,0.5); overflow: hidden; animation: fadeInBg 0.3s; }
        @keyframes fadeInBg { from { background-color: rgba(0,0,0,0); } to { background-color: rgba(0,0,0,0.5); } }
        .confirmation-content { background-color: #fff; margin: 15% auto; padding: 25px; border-radius: 8px; width: 90%; max-width: 400px; text-align: center; box-shadow: 0 5px 15px rgba(0,0,0,0.2); animation: modalPopIn 0.3s ease-out; }
        @keyframes modalPopIn { from {transform: scale(0.9) translateY(-20px); opacity: 0;} to {transform: scale(1) translateY(0); opacity: 1;} }
        ...
        /* Toast close button (hidden by default) */
        ...
        .inventory-table td:nth-child(4), .inventory-table td:nth-child(5) { text-align: center; } /* Price, Qty */
        ...
        /* Cart Quantity and Remove Button */
        ...
        /* Overlay Content General Styling */
        #addOrderOverlay .overlay-content, #inventoryOverlay .overlay-content, #cartModal .overlay-content, #orderDetailsModal .overlay-content { 
            ...
            width: 90%; max-width: 800px; /* Default, can be overridden */
        }
        ...
        /* Status Modal Content Max Height */
        ...
        #editDateModal { display: none; position: fixed; z-index: 2000; left: 0; top: 0; width: 100%; height: 100%; background-color: rgba(0,0,0,0.6); animation: fadeInBg 0.3s; }
        .edit-date-modal-content { background-color: #fff; margin: 15% auto; padding: 25px; border-radius: 8px; width: 90%; max-width: 450px; text-align: left; box-shadow: 0 5px 15px rgba(0,0,0,0.2); animation: modalPopIn 0.3s ease-out; }
        ...
        /* General Alert Styles (for session messages) */
        ...
        .main-content { padding: 25px; margin-left: 260px; /* Adjust if sidebar width changes */ transition: margin-left 0.3s; background-color: #f4f6f9; min-height: 100vh;}
        ...
        .orders-table { width: 100%; min-width: 1500px; /* Adjusted for new columns */ border-collapse: collapse; }
        ...
        .orders-table td:nth-child(7), /* Progress */
        .orders-table td:nth-child(9), /* Total Amount */
        .orders-table td:nth-child(12) /* Actions */ { text-align: center; }
        ...
        /* General Overlay & Modal Styles */
        .overlay { display: none; position: fixed; z-index: 1050; left: 0; top: 0; width: 100%; height: 100%; overflow: hidden; outline: 0; background-color: rgba(0,0,0,0.55); animation: fadeInBg 0.3s;}
        .modal { display: none; position: fixed; z-index: 1060; left: 0; top: 0; width: 100%; height: 100%; overflow: hidden; outline: 0; background-color: rgba(0,0,0,0.55); animation: fadeInBg 0.3s;}
        .modal-content { background-color: #fefefe; margin: 10% auto; padding: 25px; border: none; width: 90%; max-width: 500px; border-radius: 8px; box-shadow: 0 5px 20px rgba(0,0,0,0.2); animation: modalPopIn 0.3s ease-out;}
        ...
        /* Item Progress in Order Details Modal */
        ...
        .status-cell { vertical-align: middle; } .completed-item { background-color: #e6ffed !important; } .completed { background-color: #d1e7dd !important; } /* For unit rows */
        ...
        /* Overlay Header (for modals like Add Order, Inventory) */
        ...
        .overlay-header .cancel-btn, .overlay-header .cart-btn { font-size: 0.85em; padding: 6px 12px; } /* Smaller close buttons in overlay headers */
        /* Cart Button in Inventory Modal */
        ...
        /* Form Buttons (Save, Cancel, Back, Confirm) */
        ...
        .order-form input[type="text"], .order-form input[type="date"], .order-form select, .order-form textarea { 
            width: 100%; padding: 10px 12px; margin-bottom: 0; /* Remove direct margin-bottom from input */
        ...
        /* Add Order Modal - Order Type Specific Sections */
...
<body>
            ...
            <table class="orders-table">
                ...
                <tbody>
                    <?php if (count($orders_list) > 0): ?>
                        ...
                        <?php endforeach; ?>
                    <?php else: ?>
                        <tr><td colspan="12" class="no-orders">No orders found matching criteria.</td></tr> {/* Adjusted colspan */}
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
    ...
    <div id="addOrderOverlay" class="overlay" style="display: none;">
        <div class="overlay-content">
            ...
            <form id="addOrderForm" method="POST" class="order-form">
                ...
                <input type="hidden" name="company_name_final" id="company_name_final_for_submit"> {/* This will store the final company name for DB */}
    ...
    <script>
        ...
        function confirmPendingStatusChange(poNumber, username, ordersJsonString, originalStatus) {
            ...
            $('#activeStatusBtn').prop('disabled', true); // Disable until check is complete
            ...
            try {
                ...
                JSON.parse(ordersJsonString); // Validate JSON before sending
                $.ajax({
                    url: '/backend/check_raw_materials.php', type: 'POST', data: { orders: ordersJsonString, po_number: poNumber }, dataType: 'json',
                    success: function(response) {
                        console.log("Inventory Check (Pending Order Modal):", response);
                        if (response && typeof response === 'object') { // Basic check for valid JSON object
                            if (response.success) {
                                ...
                                updatePendingOrderActionStatus(response); // This will enable/disable 'Active' button
                            } else { 
                                ...
                                $('#activeStatusBtn').prop('disabled', false); // Allow manual override if check fails this way
                            }
                        } else {
        ...
        function confirmStatusAction(newStatus) { // Renamed status to newStatus
            ...
            if ($('#statusModal').css('display') === 'flex') $('#statusModal').css('display', 'none');
            if ($('#pendingStatusModal').css('display') === 'flex') $('#pendingStatusModal').css('display', 'none');
            if ($('#rejectedStatusModal').css('display') === 'flex') $('#rejectedStatusModal').css('display', 'none');
        }
        function closeStatusConfirmation() { 
            $('#statusConfirmationModal').css('display', 'none'); 
            // Re-show the originating status modal if user clicks "No"
        ...
        function updateOrderStatus(newStatus, deductMaterials, returnMaterials) {
            ...
            if (newStatus === 'For Delivery') { formData.append('progress', '100'); } // Ensure progress is set if moving to For Delivery

            console.log("Sending status update to backend:", Object.fromEntries(formData)); 
            ...
            .then(data => { 
                console.log("Status update response from backend:", data);
                if (data.success) { 
            ...
            .finally(() => { 
                ...
                currentPoNumber = ''; currentOrderOriginalStatus = ''; currentSelectedStatus = ''; // Reset state
            });
        }
        ...
        function closeStatusModal() { $('#statusModal').css('display', 'none'); currentSelectedStatus = ''; currentOrderOriginalStatus = ''; }
        function closeRejectedStatusModal() { $('#rejectedStatusModal').css('display', 'none'); currentSelectedStatus = ''; currentOrderOriginalStatus = ''; $('#rejectedStatusModal').removeData('po_number'); }
        function closePendingStatusModal() { $('#pendingStatusModal').css('display', 'none'); currentSelectedStatus = ''; currentOrderOriginalStatus = ''; $('#pendingStatusModal').removeData('po_number'); $('#rawMaterialsContainer').html('<p style="text-align:center; padding:10px;"><i class="fas fa-spinner fa-spin"></i> Loading inventory status...</p>'); $('#activeStatusBtn').prop('disabled', true); }
        function closeRelevantStatusModals() { closeStatusModal(); closePendingStatusModal(); closeRejectedStatusModal(); }

        // --- Material Display Helpers (for Pending Order Modal) ---
        function displayFinishedProducts(productsData, containerSelector) { 
            const container = $(containerSelector); 
            if (!container.length) { console.error("Material container not found:", containerSelector); return false; }
            ...
            if (!productsData || Object.keys(productsData).length === 0) { 
                ...
                return false; // Indicate no manufacturing seems needed based on this data
            } 
            ...
            Object.keys(productsData).forEach(productName => { 
                ...
                html += `<tr>
                            ...
                            <td class="${isSufficient ? 'material-sufficient' : 'material-insufficient'}">${isSufficient ? '<i class="fas fa-check-circle"></i> In Stock' : `<i class="fas fa-times-circle"></i> Short by ${data.shortfall || required}`}</td>
                         </tr>`; 
            }); 
            html += `</tbody></table>`; 
            container.html(html); 
            if (anyProductInsufficient) { // If any finished product is short, then we might need raw materials info
                container.append('<div id="raw-materials-section" style="margin-top:15px;"><h4>Raw Materials for Manufacturing (if applicable):</h4><p class="text-muted">Loading details if manufacturing is needed...</p></div>');
                 ...
                 container.append('<div id="raw-materials-section" style="margin-top:15px;"></div>'); // Empty div if all finished products are fine
            }
            return anyProductInsufficient; // True if manufacturing might be needed for these finished products
        }
        function displayRawMaterials(materialsData, containerSelector) { 
            const container = $(containerSelector); 
            if (!container.length) { console.error("Raw material sub-container not found:", containerSelector); return true; } // Default to sufficient if container missing
            ...
            if (!materialsData || Object.keys(materialsData).length === 0) { 
                ...
                return true; // Assume sufficient if no data or not needed
            } 
            ...
            Object.keys(materialsData).forEach(materialName => { 
                ...
                html += `<tr>
                            <td>${materialName}</td>
                            <td style="text-align:right;">${formatWeight(available)}</td>
                            <td style="text-align:right;">${formatWeight(required)}</td>
                            <td class="${isSufficient ? 'material-sufficient' : 'material-insufficient'}">${isSufficient ? '<i class="fas fa-check-circle"></i> Sufficient' : '<i class="fas fa-times-circle"></i> Insufficient'}</td>
                         </tr>`; 
            }); 
            html += `</tbody></table>`; 
        ...
        function updatePendingOrderActionStatus(inventoryResponse) {
            ...
            const materialContainer = $('#rawMaterialsContainer'); // The main container in the modal

            const finishedProducts = inventoryResponse.finishedProducts || {};
            const needsManufacturing = inventoryResponse.needsManufacturing || false; // Explicitly check this flag from backend
            ...
            if (Object.keys(finishedProducts).length === 0 && !needsManufacturing) {
                // No finished products specified OR none need manufacturing (e.g. order of only raw materials?)
                // This case might need refinement based on business logic. For now, assume can activate.
                overallStatusMessage = 'No finished products require stock check for this order. Ready to activate.';
            } else if (!allFinishedProductsInStock) { // Some finished products are short
                if (needsManufacturing) { // And backend says manufacturing is an option
                    let allRawMaterialsSufficientForMfg = Object.keys(materials).length > 0 && Object.values(materials).every(m => m.sufficient);
                     if (Object.keys(materials).length === 0 && Object.values(finishedProducts).some(p => !p.sufficient && p.canManufacture)) {
                        // Needs manufacturing but no raw material data provided for those items
                        allRawMaterialsSufficientForMfg = false; // Assume cannot manufacture if no data
                ...
                } else { // Finished products short, and manufacturing is NOT flagged as possible by backend (or no mfg items)
                    canActivateOrder = false;
        ...
        // --- Order Details Modal Functions (for Active Orders - Item Progress) ---
        // (Assuming these functions were more or less complete in the "2000 line" version)
        function viewOrderDetails(poNumber) {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    completedItemsIndices = data.completedItems || []; // Indices of items fully completed via main checkbox
                    itemQuantityProgressData = data.quantity_progress_data || {}; // { itemIndex: { unitIndex: boolean } }
                    itemProgressPercentagesMap = data.item_progress_percentages || {}; // { itemIndex: percentage }
                    currentOverallProgress = data.overall_progress || 0;

                    $('#orderStatusView').text(data.order_status || 'Active'); // Display current order status if available
                    const orderDetailsBody = $('#orderDetailsBody').empty();
                    $('#status-header-cell').show(); // Show the status/progress column for items
                    ...
                    currentOrderItemsData.forEach((item, index) => {
                        ...
                        // Item's own progress based on its units, or 100 if main checkbox was checked & no units
                        let currentItemUnitProgress = itemQty > 0 ? (unitsCompletedCount / itemQty) * 100 : (completedItemsIndices.includes(index) ? 100 : 0);
                        itemProgressPercentagesMap[index] = currentItemUnitProgress; // Store initial
                    ...
                    currentOverallProgress = calculatedOverallProgress; // Use calculated based on current state
        ...
        function viewOrderInfo(ordersJsonString, orderStatus, poNumber) {
            try {
                ...
                $('#status-header-cell').hide(); // Hide item progress column for non-active
                    ...
                    orderDetailsArray.forEach(p => {
                        ...
                        itemsBody.append(
                           `<tr>
                                ...
                                <td>${p.packaging || 'N/A'}</td>
                                <td style="text-align:right;">PHP ${(parseFloat(p.price) || 0).toFixed(2)}</td>
                                <td style="text-align:center;">${p.quantity || 0}</td>
                                <td>N/A (Status: ${orderStatus})</td>
                            </tr>`
                        );
                    });
                } else {
                ...
                $('#overall-progress-info, .save-progress-btn').hide(); // Hide progress bar and save button
                $('#orderDetailsModal').css('display', 'flex');
            } catch (e) {
        ...
        function updateUnitStatus(checkbox) {
            ...
            if (!itemQuantityProgressData[itemIndex]) { 
                ...
                // Initialize all units for this item if not already
            ...
            updateOverallProgress(); // This will also update its display
        }
        function updateItemProgressUI(itemIndex) { // Renamed from updateItemProgress
            const item = currentOrderItemsData[itemIndex]; 
            const qty = parseInt(item.quantity) || 0; 
            if (qty === 0) { // For items with no quantity (rely on main checkbox)
            ...
            // Update main item row style if all units are complete (or main checkbox for qty 0 items)
            const isItemNowFullyComplete = progressPercent >= 99.9; // Use a threshold for float precision
            $(`tr.item-header-row[data-item-index="${itemIndex}"]`).toggleClass('completed-item', isItemNowFullyComplete);
            // If item has 0 quantity, its "completed" status is tied to the main checkbox, not unit progress
            if (qty === 0) {
                 const mainCheckbox = $(`.item-status-checkbox[data-index="${itemIndex}"]`);
                 if (mainCheckbox.length) { // Check if checkbox exists
        ...
        function updateOverallProgress() {
            ...
            updateOverallProgressDisplay(); 
            return Math.round(currentOverallProgress); // Return rounded for immediate use if needed
        }
        ...
        function updateRowStyle(mainCheckbox) { // For items with Qty 0, controlled by a single checkbox
            ...
            itemProgressPercentagesMap[itemIndex] = isChecked ? 100 : 0; // Set item progress directly
            // Update completedItemsIndices for items with Qty 0
            ...
            updateItemProgressUI(itemIndex); // Reflect change in progress bar if any
            updateOverallProgress();
        }
        function closeOrderDetailsModal() { $('#orderDetailsModal').css('display', 'none'); currentOrderItemsData = []; /* Clear data */ }
        function confirmSaveProgress() { $('#saveProgressConfirmationModal .confirmation-message').text('Are you sure you want to save the current item completion progress for PO: ' + currentPoNumber + '?'); $('#saveProgressConfirmationModal').css('display', 'block'); }
        ...
        function saveProgressChanges() {
            ...
            const finalCalculatedOverallProgress = updateOverallProgress(); // Ensure overall progress is up-to-date

            const payload = {
                po_number: currentPoNumber,
                completed_items_indices: completedItemsIndices, // For items with qty 0, if used
                quantity_progress_data: itemQuantityProgressData, // For items with qty > 0
            ...
            if (finalCalculatedOverallProgress >= 100) {
                ...
                // Backend's update_order_status should also handle setting progress to 100 if status becomes 'For Delivery'
                updateOrderStatus('For Delivery', false, false); // No material changes here, just status and progress
                // The updateOrderStatus function will handle reload and further notifications
            } else {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        // Optionally, reload the main table row for this PO to reflect new overall progress
                        // Or update it client-side if the main table progress bar has an ID
                        setTimeout(() => { 
                            // Find the row and update its progress bar if possible, or just reload
                            // For simplicity here, we'll just close modal. Main table reloads on full page load.
                            closeOrderDetailsModal();
                            // To refresh just the table row, more complex JS would be needed to find and update the specific row's progress bar.
                            // A full reload is safer if other derived data changes.
                             window.location.reload(); // Reload to show updated progress in main table
                        }, 1500);
                    } else {
        ...
        function openEditDateModal(poNumber, currentDeliveryDate, orderDate) { 
            currentPoNumber = poNumber; 
            currentEditingOrderDate = orderDate; // Store the order date for validation
            $('#edit_po_number').val(poNumber); 
            $('#current_delivery_date').val(currentDeliveryDate ? new Date(currentDeliveryDate).toLocaleDateString('en-CA') : 'N/A'); // YYYY-MM-DD
            ...
            $('#new_delivery_date_input').val(''); // Clear previous value

            if ($.datepicker) { 
                $("#new_delivery_date_input").datepicker("destroy"); 
                let minDateForPicker = new Date(orderDate); // Start from order date
                minDateForPicker.setDate(minDateForPicker.getDate() + 5); // Add 5 days minimum
        ...
        function confirmDownloadPO(poNumber, username, company, orderDate, deliveryDate, deliveryAddress, ordersJsonString, totalAmount, specialInstructions) { 
            ...
            console.log("Data prepared for PDF:", poDownloadDataStore); 
        ...
        function downloadPODirectly() { 
            ...
            try { 
                currentPODataForPDF = poDownloadDataStore; // Use the stored data
                ...
                $('#pdfPreview').show(); // Show preview container briefly if needed, or skip to download
                ...
                .then(() => { 
                    ...
                    // closePDFPreview(); // Optionally close preview after download
                })
                .catch(err => { 
                ...
                .finally(() => {
                    currentPODataForPDF = null; // Clear data after attempt
                    poDownloadDataStore = null;
                    // Keep pdfPreview open or close it:
                    // setTimeout(closePDFPreview, 500); // Example: close after a short delay
                });
        ...
        function downloadPDF() { /* This is called by button inside pdfPreview, main logic in downloadPODirectly now */ 
        ...
        function viewSpecialInstructions(poNumber, instructions) { 
            ...
            if (instructions && instructions.trim()) { 
                contentElement.html(instructions.replace(/\n/g, '<br>')).removeClass('empty'); // Preserve line breaks
            } else { 
        ...
        // --- Add New Order Form Logic (Order Type, Conditional Fields, etc.) ---
        function initializeDeliveryDatePickerForAddOrder(baseDateStr = null) {
            ...
            if ($.datepicker && deliveryInput.length) {
                deliveryInput.datepicker("destroy"); // Clear previous instance

                let minDateForPicker;
                if (baseDateStr) {
                    minDateForPicker = new Date(baseDateStr);
                } else { // If no base date (e.g. on initial load before order date is set), use today
                    minDateForPicker = new Date();
                }
                minDateForPicker.setDate(minDateForPicker.getDate() + 5); // Minimum 5 days from base/today
                ...
                deliveryInput.datepicker({
                    ...
                    onSelect: function(dateText) {
                        // Optional: any action on date select
                        console.log("Delivery date selected:", dateText);
                    }
                });
                // Set a default valid date if input is empty
        ...
        function toggleOrderFormFields() {
            ...
            $('#order_type_hidden_for_submit').val(selectedOrderType); // Set hidden input for form submission

            // Reset/hide all sections first
            $('#onlineSpecificInputs, #walkInSpecificInputs, #commonOrderFields, #confirmAddOrderBtn').hide();
            $('#username_online_select').val(''); // Reset client dropdown
            $('#online_company_display').val('');
            $('#walk_in_name_company_input').val('');
            $('#company_address_option_for_delivery').hide(); // Hide company address option by default

            if (selectedOrderType === "Online") {
                $('#onlineSpecificInputs').slideDown(200);
                $('#commonOrderFields').slideDown(200);
                $('#confirmAddOrderBtn').show();
                $('#company_address_option_for_delivery').show(); // Show option to use client's address
                $('#delivery_address_type_select').val('company'); // Default to company address
                handleOnlineUserChange(); // Populate company if user already selected, or clear if not
            } else if (selectedOrderType === "Walk In") {
                $('#walkInSpecificInputs').slideDown(200);
                $('#commonOrderFields').slideDown(200);
                $('#confirmAddOrderBtn').show();
                $('#delivery_address_type_select').val('custom'); // Default to custom address
                $('#po_number_for_submit').val(generateWalkInPONumber());
                $('#company_name_final_for_submit').val(''); // Clear any potential company from online user
            }
            
            // Common fields initialization if an order type is selected
            if (selectedOrderType) {
                const today = new Date();
                const formattedToday = `${today.getFullYear()}-${(today.getMonth()+1).toString().padStart(2,'0')}-${today.getDate().toString().padStart(2,'0')}`;
                $('#order_date_input').val(formattedToday);
                initializeDeliveryDatePickerForAddOrder(formattedToday); // Pass order date as base
                toggleDeliveryAddressOptions(); // Update address fields based on default selection
            }
            // Reset cart and summary for new order type selection
            currentCartItems = [];
            updateOrderSummary();
            updateCartItemCount();
        }
        ...
        function handleOnlineUserChange() {
            ...
            if (username) {
                ...
                $('#company_name_final_for_submit').val(companyName); // For DB
                $('#company_address_display_field').val(companyAddr); 
                
                if ($('#delivery_address_type_select').val() === 'company') {
                     $('#delivery_address_for_submit').val(companyAddr); // Auto-fill if company address selected
                }
                $('#po_number_for_submit').val(generateOnlinePONumber(username));
            } else { // No user selected or "--Select--"
                $('#online_company_display').val('');
                ...
                if ($('#delivery_address_type_select').val() === 'company') {
                 ...
                 $('#po_number_for_submit').val(''); // Clear PO if no user
            }
        }
        ...
        function toggleDeliveryAddressOptions() {
            ...
            if (deliveryTypeSelected === 'company' && currentOrderType === 'Online') {
                $('#company_address_container_div').slideDown(150);
                $('#custom_address_container_div').slideUp(150);
                // Value for submission is taken from the readonly field, updated by handleOnlineUserChange
                $('#delivery_address_for_submit').val($('#company_address_display_field').val()); 
            } else { // Custom address for Walk-In or if 'Custom' is chosen for Online
                $('#company_address_container_div').slideUp(150);
                $('#custom_address_container_div').slideDown(150);
                $('#delivery_address_for_submit').val($('#custom_address_input_field').val());
            }
             // Ensure company address option is hidden if not Online type
            if (currentOrderType !== 'Online') {
                $('#company_address_option_for_delivery').hide();
                if (deliveryTypeSelected === 'company') { // If it was company, switch to custom
        ...
        $('#custom_address_input_field').on('input', function() { // Update hidden field if custom address is typed
        ...
        function openAddOrderForm() {
            $('#addOrderForm')[0].reset(); // Reset all form fields to their default values
            currentCartItems = []; // Clear the cart
            updateOrderSummary(); // Update the summary table display
            updateCartItemCount(); // Update cart icon counters
            
            $('#order_type_selection').val("").trigger('change'); // Reset order type and trigger field toggle
            // Explicitly hide sections that might not be reset by toggle if no val()
            $('#onlineSpecificInputs, #walkInSpecificInputs, #commonOrderFields, #confirmAddOrderBtn').hide();
            
            $('#addOrderOverlay').css('display', 'flex'); // Show the modal
        }
        
        function generateOnlinePONumber(username) {
            if (!username) return '';
            const d = new Date();
            // Format: PO-USER(UPTO4)-YYMMDD-HHMM-RAND3
        ...
        function prepareOrderDataForSubmit() { // Renamed for clarity
            const selectedOrderType = $('#order_type_selection').val();
            ...
            if (selectedOrderType === "Online") {
                ...
                finalCompanyForDB = $('#online_company_display').val(); // This is derived from selected client
            } else if (selectedOrderType === "Walk In") {
                finalUsernameForDB = 'Walk-In Customer'; // Or a generated placeholder, or null if DB allows
            ...
            $('#company_name_final_for_submit').val(finalCompanyForDB); // Set the hidden field for company
            ...
            // Delivery Address (already set in #delivery_address_for_submit by toggleDeliveryAddressOptions)
            ...
            // PO Number (already set in #po_number_for_submit by generation functions)
        ...
        function confirmAddOrder() {
            if (prepareOrderDataForSubmit()) { // Call the validation/preparation function
        ...
        function submitAddOrder() {
            ...
            if (!prepareOrderDataForSubmit()) return; // Final validation check

            const formElement = document.getElementById('addOrderForm');
            const formData = new FormData(formElement);
            
            // Explicitly set username based on order type for submission if needed,
            // or ensure backend handles 'username_online' vs. a default for walk-in.
            const orderType = formData.get('order_type'); // From the hidden field
            if (orderType === "Walk In") {
                // Backend should use company_name_final and a placeholder for username if 'username_online' is not present
                formData.delete('username_online'); // Ensure it's not sent if it was in the DOM
                formData.append('username_placeholder_for_walkin', 'Walk-In Customer'); // Example, backend can use this or default
            }
            // For 'Online', 'username_online' is already part of formData due to its name attribute.

            console.log("Submitting Add Order FormData:", Object.fromEntries(formData));
            ...
            .then(data => {
                if (data.success) {
                    ...
                    setTimeout(() => { window.location.href = 'orders.php'; }, 2000); // Redirect after showing success
                } else {
        ...
        function openInventoryOverlay() { 
            ...
            .then(data => { 
                if (data.success && data.inventory) { 
                    populateInventoryTable(data.inventory); // Renamed
                    populateCategoryFilter(data.categories || []); // Renamed
                } else { 
        ...
        function populateInventoryTable(inventoryItems) { // Renamed
            ...
            inventoryItems.forEach(item => { 
                ...
                inventoryBody.append(
                   `<tr>
                        ...
                        <td>${item.item_description || 'N/A'}</td>
                        <td>${item.packaging || 'N/A'}</td>
                        <td style="text-align:center;">PHP ${price.toFixed(2)}</td>
                        <td style="text-align:center;"><input type="number" class="inventory-quantity form-control form-control-sm" value="1" min="1" max="100" style="width:70px; margin:auto;"></td>
                        <td style="text-align:center;"><button class="add-to-cart-btn btn btn-primary btn-sm" onclick="addToCartFromInventory(this, '${item.product_id}', '${item.category||''}', '${item.item_description||''}', '${item.packaging||''}', ${price})"><i class="fas fa-cart-plus"></i> Add</button></td>
                    </tr>`
                ); 
            }); 
        }
        function populateCategoryFilter(categories) { // Renamed
            const categorySelect = $('#inventoryFilter'); 
            categorySelect.find('option:not(:first-child)').remove(); // Clear old options except "All"
        ...
        $('#inventorySearch, #inventoryFilter').off('input change', filterInventory).on('input change', filterInventory); // Rebind with .off
        function closeInventoryOverlay() { $('#inventoryOverlay').hide(); }
        function addToCartFromInventory(button, productId, category, itemDesc, packaging, price) { // Renamed
            ...
            const existingCartItemIndex = currentCartItems.findIndex(i => i.product_id == productId && i.packaging === packaging); // Use == for product_id if it's string from DOM
            ...
            if (existingCartItemIndex >= 0) {
                 ...
                 if (newTotalQuantity > 100) {
                     ...
                     // Optionally, you could cap it at 100 or prevent adding. Here, we prevent adding if it exceeds.
                     // currentCartItems[existingCartItemIndex].quantity = 100; // Or cap
                     return; // Prevent adding if combined exceeds
                 } else {
            ...
            quantityInput.val(1); // Reset inventory input for next item
            updateOrderSummary(); // Update the main form's summary
            updateCartItemCount(); // Update all cart count indicators
        }
        function updateOrderSummary() { 
                ...
                currentCartItems.forEach((item, index) => { 
                    ...
                    summaryTableBody.append(
                       `<tr>
                            ...
                            <td>${item.packaging}</td>
                            <td style="text-align:right;">PHP ${parseFloat(item.price || 0).toFixed(2)}</td>
                            <td style="text-align:center;"><input type="number" class="cart-quantity summary-quantity form-control form-control-sm" value="${item.quantity}" min="1" max="100" data-cart-item-index="${index}" onchange="updateSummaryItemQuantity(this)" style="width:70px; margin:auto;"></td>
                            <td style="text-align:center;"><button class="remove-item-btn btn btn-danger btn-sm" onclick="removeSummaryItem(${index})"><i class="fas fa-trash-alt"></i></button></td>
                        </tr>`
                    ); 
                }); 
            ...
            $('#total_amount_for_submit').val(currentOrderTotal.toFixed(2)); // Also update hidden input for form
        }
        function updateSummaryItemQuantity(inputElement) { 
            ...
            if (isNaN(newQuantity) || newQuantity < 1) { showToast('Quantity must be at least 1.', 'error'); $(inputElement).val(currentCartItems[itemIndexInCart].quantity); /* Revert */ return; }
            ...
            updateOrderSummary(); // Refresh summary table which recalculates total
            updateCartItemCount(); 
            updateCartDisplay(); // Also update the cart modal if it's open
        }
        function removeSummaryItem(itemIndexInCart) { 
            if (itemIndexInCart >= 0 && itemIndexInCart < currentCartItems.length) { 
                ...
                updateOrderSummary(); 
                updateCartItemCount(); 
                updateCartDisplay(); // Sync with cart modal
            } 
        }
        function updateCartItemCount() { 
            const count = currentCartItems.length;
            $('#cartItemCount, #cartItemCountNav, #orderSummaryItemCount').text(`(${count} item${count === 1 ? '' : 's'})`); 
        ...
        function saveCartChangesAndClose() { /* This function is effectively a "confirm" from cart modal */
            updateOrderSummary(); // Ensure main form summary is synced with cart modal changes
            closeCartModal(); 
            showToast('Selected items confirmed.', 'success');
        }
        function updateCartDisplay() { 
            const cartTableBody = $('.cart').empty(); // Target tbody with class 'cart'
            const noProductsRow = $('.no-products-in-cart-row'); // Target the specific row for "no products"
            const cartTotalElement = $('.total-amount-cart'); 
            let currentCartModalTotal = 0; 

            if (currentCartItems.length === 0) { 
                if(noProductsRow.length) noProductsRow.show(); // Show if exists
                ...
                if(noProductsRow.length) noProductsRow.hide(); // Hide if exists
                currentCartItems.forEach((item, index) => { 
                    ...
                    cartTableBody.append(
                       `<tr>
                            ...
                            <td>${item.item_description}</td>
                            <td>${item.packaging}</td>
                            <td style="text-align:center;">PHP ${(parseFloat(item.price) || 0).toFixed(2)}</td>
                            <td style="text-align:center;"><input type="number" class="cart-quantity form-control form-control-sm" value="${item.quantity}" min="1" max="100" data-cart-item-index="${index}" onchange="updateCartModalItemQuantity(this)" style="width:70px; margin:auto;"></td>
                            <td style="text-align:center;"><button class="remove-item-btn btn btn-danger btn-sm" onclick="removeCartModalItem(${index})"><i class="fas fa-trash-alt"></i></button></td>
                        </tr>`
                    ); 
                }); 
        ...
        function updateCartModalItemQuantity(inputElement) { // Specifically for cart modal
            const itemIndexInCart = parseInt($(inputElement).data('cart-item-index')); 
            let newQuantity = parseInt($(inputElement).val()); 
            ...
            currentCartItems[itemIndexInCart].quantity = newQuantity; 
            updateCartDisplay(); // Update total in cart modal itself
            updateOrderSummary(); // Also sync with main form's summary
        }
        function removeCartModalItem(itemIndexInCart) { // Specifically for cart modal
            if (itemIndexInCart >= 0 && itemIndexInCart < currentCartItems.length) { 
                const removedItem = currentCartItems.splice(itemIndexInCart, 1)[0]; 
                showToast(`Removed ${removedItem.item_description} from selection.`, 'info'); 
                updateCartDisplay(); 
                updateOrderSummary(); // Sync with main form
                updateCartItemCount(); 
            } 
        }
        
        // --- Document Ready & General Event Handlers ---
        $(document).ready(function() {
            ...
            // $(".search-btn").on("click", () => $("#searchInput").trigger("input")); // Not really needed if using keyup/input

            // Initial call for Add Order Modal's delivery address options (will be re-called on type change)
            // toggleDeliveryAddressOptions(); // Now handled by toggleOrderFormFields
            
            // Global click listener for closing modals/overlays when clicking outside content
            $(document).on('click', function(event) {
                // For overlays with .overlay-content
                ...
                // For modals with .modal-content (or specific content classes)
                if ($(event.target).hasClass('modal')) {
                     if (!$(event.target.children[0]).is(event.target) && $(event.target.children[0]).hasClass('modal-content') || $(event.target.children[0]).hasClass('edit-date-modal-content') || $(event.target.children[0]).hasClass('instructions-modal-content')) {
                        // Click was on modal backdrop, not content
                        ...
                        else if (modalId === 'specialInstructionsModal') closeSpecialInstructions();
                        else if ($(event.target).hasClass('confirmation-modal')) $(event.target).hide(); // General hide for confirmation modals
                     }
                }
            });
            
            // Initialize UI elements if needed (e.g. if not done by specific open functions)
            // For example, ensure order type selection is blank on page load for the add order form.
            $('#order_type_selection').val(""); 
            $('#onlineSpecificInputs, #walkInSpecificInputs, #commonOrderFields').hide();
        });
        
        // Optional: Email sending status check (if you implement check_email_status.php)
        /*
        function checkEmailSendingStatus() { 
            fetch('/backend/check_email_status.php') // Replace with your actual path
            .then(response => response.json())
            .then(data => { 
                if (data.unsent_notifications > 0) { 
                    showToast(`${data.unsent_notifications} email notifications are pending in the queue.`, 'info', 5000); 
                }
            })
            .catch(error => { 
                console.warn("Could not check email sending status:", error); 
            });
        }
        // document.addEventListener('DOMContentLoaded', function() { 
        //     setTimeout(checkEmailSendingStatus, 5000); // Check a bit after page load
        //     setInterval(checkEmailSendingStatus, 180000); // Check every 3 minutes
        // });
        */
    </script>
</body>
