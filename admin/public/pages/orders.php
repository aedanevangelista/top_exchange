...
// NOTE: $conn->close() is intentionally NOT called here to allow sidebar.php to use the connection.
...
<head>
    ...
    <style>
        ...
        /* Add styles from previous version if missing */
        ...
        .orders-table thead th { background-color:rgb(29, 29, 29); font-weight: 600; position: sticky; top: 0; z-index: 10; }
        ...
        .action-buttons button, .view-orders-btn, .driver-btn { padding: 5px 10px; font-size: 12px; border-radius: 4px; cursor: pointer; border: none; display: inline-flex; align-items: center; gap: 3px; }
        ...
        .driver-btn { background-color: #6c757d; color: white; }
        .driver-btn.assign-driver-btn { background-color: #198754; } /* Green for assign */
        .driver-btn:hover { background-color: #5a6268; }
        .driver-btn.assign-driver-btn:hover { background-color: #157347; }
        .driver-badge { padding: 5px 10px; border-radius: 4px; font-size: 12px; display: inline-block; background-color: #e9ecef; color: #495057; border: 1px solid #ced4da; }
        .driver-badge.driver-not-assigned { background-color: #fff3cd; color: #856404; border-color: #ffeeba; }
        .driver-badge i { margin-right: 5px; }
        ...
        .driver-modal-content { max-width: 400px; }
...
<body>
    ...
    <div id="statusModal" class="modal" style="display: none;">
        <div class="modal-content">
            ...
            <div class="status-buttons"><button onclick="confirmStatusAction('For Delivery')" class="modal-status-btn delivery"><i class="fas fa-truck"></i> For Delivery<div class="btn-info">(Requires 100% & Driver)</div></button><button onclick="confirmStatusAction('Pending')" class="modal-status-btn pending"><i class="fas fa-clock"></i> Pending<div class="btn-info">(Return stock)</div></button><button onclick="confirmStatusAction('Rejected')" class="modal-status-btn rejected"><i class="fas fa-times-circle"></i> Reject<div class="btn-info">(Return stock)</div></button></div>
    ...
    <div id="pendingStatusModal" class="modal" style="display: none;">
        <div class="modal-content">
            ...
            <div class="status-buttons"><button id="activeStatusBtn" onclick="confirmStatusAction('Active')" class="modal-status-btn active" disabled><i class="fas fa-check"></i> Active<div class="btn-info">(Deduct stock)</div></button><button onclick="confirmStatusAction('Rejected')" class="modal-status-btn rejected"><i class="fas fa-times-circle"></i> Reject</button></div>
    ...
    <script>
        ...
        function confirmDriverChange(poNumber, driverId, driverName) { currentPoNumber = poNumber; currentDriverId = driverId; $('#driverModalTitle').text('Change Driver'); $('#driverModalMessage').text(`Change driver for ${poNumber}? Current: ${driverName}`); $('#driverSelect').val(driverId); $('#driverModal').css('display', 'flex'); }
        ...
        function confirmDriverAssignment() {
            ...
            const name = $('#driverSelect option:selected').text(); let msg = `Assign driver ${name}?`; if (currentDriverId > 0 && currentDriverId !== driverId) msg = `Change driver to ${name}?`; else if (currentDriverId === driverId) { showToast('No change detected', 'info'); closeDriverModal(); return; }
        ...
        function downloadPDF() { /* Keep existing downloadPDF function for preview modal */ if (!currentPOData) { showToast('No data', 'error'); return; } const element = document.getElementById('contentToDownload'); const opt = { margin: [10,10,10,10], filename: `PO_${currentPOData.poNumber}_Preview.pdf`, image: { type: 'jpeg', quality: 0.98 }, html2canvas: { scale: 2 }, jsPDF: { unit: 'mm', format: 'a4', orientation: 'portrait' } }; html2pdf().set(opt).from(element).save(); }
        ...
        function toggleDeliveryAddress() { const type = $('#delivery_address_type').val(); $('#company_address_container').toggle(type === 'company'); $('#custom_address_container').toggle(type === 'custom'); $('#delivery_address').val(type === 'company' ? $('#company_address').val() : $('#custom_address').val()); }
        ...
        function submitAddOrder() {
             ...
             fetch('/backend/add_order.php', { method: 'POST', body: fd }).then(r => r.json()).then(d => { if (d.success) { showToast('Order added!', 'success'); closeAddOrderForm(); setTimeout(() => { window.location.reload(); }, 1500); } else { showToast('Error: ' + (d.message || 'Failed to add order'), 'error'); } }).catch(e => { console.error("Add order error:", e); showToast('Network error adding order.', 'error'); });
        }

        // --- Inventory Overlay and Cart Functions (Using product_id) ---
        function openInventoryOverlay() {
            $('#inventoryOverlay').css('display', 'flex'); const body = $('.inventory').html('<tr><td colspan="6" style="text-align:center;padding:20px;"><i class="fas fa-spinner fa-spin"></i> Loading inventory...</td></tr>');
            fetch('/backend/get_inventory.php').then(r => { if (!r.ok) throw new Error(`HTTP ${r.status}`); return r.text().then(t => { try { return JSON.parse(t); } catch (e) { console.error("Inv JSON Parse Error:", e, "Response:", t); throw new Error("Invalid inventory data format received."); } }); })
            .then(data => {
            ...
            .catch(e => { console.error('Inv fetch:', e); showToast('Inv fetch error: ' + e.message, 'error'); body.html(`<tr><td colspan="6" style="text-align:center;padding:20px;color:red;">Error loading inventory. Check console.</td></tr>`); });
        }
        ...
        function populateInventory(inventory) {
            ...
            inventory.forEach(item => {
                ...
                body.append(`<tr><td>${item.category||'Uncategorized'}</td><td>${item.item_description}</td><td>${item.packaging||'N/A'}</td><td>PHP ${price.toFixed(2)}</td><td><input type="number" class="inventory-quantity" value="1" min="1"></td><td><button class="add-to-cart-btn" onclick="addToCart(this, ${item.product_id}, '${item.category||''}', '${item.item_description}', '${item.packaging||''}', ${price})"><i class="fas fa-plus"></i> Add</button></td></tr>`);
             });
        }
        ...
        function filterInventory() {
            ...
            $('.inventory tr').each(function() { const row = $(this); if (row.find('th').length > 0 || (row.find('td').length === 1 && row.find('td').attr('colspan') === '6')) return; const cCell = row.find('td:first-child').text(); const text = row.text().toLowerCase(); const catMatch = (cat === 'all' || cCell === cat); const searchMatch = (text.includes(search)); row.toggle(catMatch && searchMatch); });
        }
        ...
        function updateOrderSummary() {
            ...
            else { cartItems.forEach((item, index) => { total += item.price * item.quantity; body.append(`<tr><td>${item.category}</td><td>${item.item_description}</td><td>${item.packaging}</td><td>PHP ${item.price.toFixed(2)}</td><td><input type="number" class="cart-quantity summary-quantity" value="${item.quantity}" min="1" data-index="${index}" onchange="updateSummaryItemQuantity(this)"></td><td><button class="remove-item-btn" onclick="removeSummaryItem(${index})"><i class="fas fa-trash"></i></button></td></tr>`); }); }
        ...
        function updateCartDisplay() {
            ...
            msg.hide(); cartItems.forEach((item, index) => { total += item.price * item.quantity; body.append(`<tr><td>${item.category}</td><td>${item.item_description}</td><td>${item.packaging}</td><td>PHP ${item.price.toFixed(2)}</td><td><input type="number" class="cart-quantity" value="${item.quantity}" min="1" data-index="${index}" onchange="updateCartItemQuantity(this)"></td><td><button class="remove-item-btn" onclick="removeCartItem(${index})"><i class="fas fa-trash"></i></button></td></tr>`); });
