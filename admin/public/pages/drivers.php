...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax'])) {
    ...
    if ($_POST['formType'] == 'add') {
        // ... (Add logic remains the same, status defaults to 'Active') ...
        ...
        // Validations (Name, Username, Contact, Address, Availability, Area)
        ...
        else { /* Username uniqueness check */
            $checkUserStmt = $conn->prepare("SELECT id FROM drivers WHERE username = ?");
            if ($checkUserStmt) { $checkUserStmt->bind_param("s", $username); $checkUserStmt->execute(); $checkUserStmt->store_result(); if ($checkUserStmt->num_rows > 0) { $errors['username'] = 'Username already exists.'; } $checkUserStmt->close(); }
            else { error_log("Add Driver Username Check Prepare Error: " . $conn->error); returnJsonResponse(false, false, 'Database error checking username.', ['general' => 'Database error checking username.']); }
        }
        ...
        if (empty($errors['name'])) { /* Name uniqueness check */
            $checkNameStmt = $conn->prepare("SELECT id FROM drivers WHERE name = ?");
            if ($checkNameStmt) { $checkNameStmt->bind_param("s", $name); $checkNameStmt->execute(); $checkNameStmt->store_result(); if ($checkNameStmt->num_rows > 0) { $errors['name'] = 'Driver name already exists.'; } $checkNameStmt->close(); }
            else { error_log("Add Driver Name Check Prepare Error: " . $conn->error); returnJsonResponse(false, false, 'Database error checking name.', ['general' => 'Database error checking name.']); }
        }
        if (!empty($errors)) { returnJsonResponse(false, false, 'Please correct the errors below.', $errors); }

        // Generate and Hash Password
        $last_four_digits = substr($contact_no, -4); $generated_password = $username . $last_four_digits;
        ...
        // Insert (Status defaults to 'Active')
        ...
        if ($stmtAdd->execute()) { returnJsonResponse(true, true, 'Driver added successfully.'); }
        else { error_log("Add Driver DB Error: " . $stmtAdd->error); returnJsonResponse(false, false, 'Failed to add driver.', ['general' => 'Database execution error.']); }
        $stmtAdd->close();
    }
    // --- EDIT DRIVER ---
    elseif ($_POST['formType'] == 'edit') {
        // ... (Edit logic remains the same, includes updating 'status' from the form) ...
        $id = $_POST['id'];
        $name = trim($_POST['name']);
        $username = trim($_POST['username']);
        ...
        $availability = $_POST['availability'];
        $area = $_POST['area'];
        $status = $_POST['status']; // Read status from form
        $errors = [];

        // Validations (Name, Username, Password, Contact, Address, Availability, Area, Status)
        if (empty($id) || !filter_var($id, FILTER_VALIDATE_INT)) $errors['general'] = 'Invalid Driver ID.';
        if (empty($name)) { $errors['name'] = 'Name is required.'; } elseif (strlen($name) > 40) { $errors['name'] = 'Name cannot exceed 40 characters.'; }
        if (empty($username)) { $errors['username'] = 'Username is required.'; } elseif (strlen($username) > 15) { $errors['username'] = 'Username cannot exceed 15 characters.'; } elseif (str_contains($username, ' ')) { $errors['username'] = 'Username cannot contain spaces.'; }
        else { /* Username uniqueness check (excluding self) */
            $checkUserStmt = $conn->prepare("SELECT id FROM drivers WHERE username = ? AND id != ?");
            if ($checkUserStmt) { $checkUserStmt->bind_param("si", $username, $id); $checkUserStmt->execute(); $checkUserStmt->store_result(); if ($checkUserStmt->num_rows > 0) { $errors['username'] = 'Username already exists.'; } $checkUserStmt->close(); }
            else { error_log("Edit Driver Username Check Prepare Error: " . $conn->error); returnJsonResponse(false, false, 'Database error checking username.', ['general' => 'Database error checking username.']); }
        }
        ...
        if (!in_array($availability, ['Available', 'Not Available'])) $errors['availability'] = 'Invalid availability status.';
        if (!in_array($area, ['North', 'South'])) $errors['area'] = 'Invalid area.';
        if (!in_array($status, ['Active', 'Archive'])) $errors['status'] = 'Invalid account status.'; // Validate status
        if (empty($errors['name']) && empty($errors['general'])) { /* Name uniqueness check (excluding self) */
             $checkNameStmt = $conn->prepare("SELECT id FROM drivers WHERE name = ? AND id != ?");
             if ($checkNameStmt) { $checkNameStmt->bind_param("si", $name, $id); $checkNameStmt->execute(); $checkNameStmt->store_result(); if ($checkNameStmt->num_rows > 0) { $errors['name'] = 'Driver name already exists.'; } $checkNameStmt->close(); }
             else { error_log("Edit Driver Name Check Prepare Error: " . $conn->error); returnJsonResponse(false, false, 'Database error checking name.', ['general' => 'Database error checking name.']); }
        }
        if (!empty($errors)) { returnJsonResponse(false, false, 'Please correct the errors below.', $errors); }

        // Prepare Update (includes status)
        $sql_update = "UPDATE drivers SET name = ?, username = ?, address = ?, contact_no = ?, availability = ?, area = ?, status = ?";
        $types = "sssssss"; $params_update = [$name, $username, $address, $contact_no, $availability, $area, $status];
        if (!empty($password)) { /* Add password update if provided */
            $hashed_password = password_hash($password, PASSWORD_DEFAULT);
            if ($hashed_password === false) { error_log("Password hashing failed for edit driver ID: $id"); returnJsonResponse(false, false, 'Failed to secure new password.', ['password' => 'Hashing failed.']); }
            $sql_update .= ", password = ?"; $types .= "s"; $params_update[] = $hashed_password;
        }
        $sql_update .= " WHERE id = ?"; $types .= "i"; $params_update[] = $id;

        // Execute Update
        $stmtEdit = $conn->prepare($sql_update);
        if (!$stmtEdit) { error_log("Edit Driver Prepare Error: " . $conn->error); returnJsonResponse(false, false, 'Failed to prepare update.', ['general' => 'Database error.']); }
        $stmtEdit->bind_param($types, ...$params_update);
        if ($stmtEdit->execute()) { returnJsonResponse(true, true, 'Driver updated successfully.'); }
        else { error_log("Edit Driver DB Error: " . $stmtEdit->error); returnJsonResponse(false, false, 'Failed to update driver.', ['general' => 'Database execution error.']); }
        $stmtEdit->close();
    }
    // --- CHANGE AVAILABILITY STATUS --- (Renamed formType)
    elseif ($_POST['formType'] == 'availability_status') {
        $id = $_POST['id'];
        $availability_status = $_POST['availability']; // Read 'availability' from POST

        if (empty($id) || !filter_var($id, FILTER_VALIDATE_INT)) { returnJsonResponse(false, false, 'Invalid Driver ID.'); }
        if (!in_array($availability_status, ['Available', 'Not Available'])) { returnJsonResponse(false, false, 'Invalid availability status provided.'); }

        $stmtAvail = $conn->prepare("UPDATE drivers SET availability = ? WHERE id = ?"); // Update 'availability' column
        if (!$stmtAvail) { error_log("Change Driver Availability Prepare Error: " . $conn->error); returnJsonResponse(false, false, 'Failed to prepare availability update.'); }
        $stmtAvail->bind_param("si", $availability_status, $id);

        if ($stmtAvail->execute()) { returnJsonResponse(true, true, 'Availability updated successfully.'); }
        else { error_log("Change Driver Availability DB Error: " . $stmtAvail->error); returnJsonResponse(false, false, 'Failed to change availability. Database error.'); }
        $stmtAvail->close();
    }
    // --- CHANGE ACCOUNT STATUS --- (New formType)
    elseif ($_POST['formType'] == 'account_status') {
        $id = $_POST['id'];
        $account_status = $_POST['status']; // Read 'status' from POST

        if (empty($id) || !filter_var($id, FILTER_VALIDATE_INT)) { returnJsonResponse(false, false, 'Invalid Driver ID.'); }
        if (!in_array($account_status, ['Active', 'Archive'])) { returnJsonResponse(false, false, 'Invalid account status provided.'); }

        $stmtAccStatus = $conn->prepare("UPDATE drivers SET status = ? WHERE id = ?"); // Update 'status' column
        if (!$stmtAccStatus) { error_log("Change Driver Account Status Prepare Error: " . $conn->error); returnJsonResponse(false, false, 'Failed to prepare account status update.'); }
        $stmtAccStatus->bind_param("si", $account_status, $id);

        if ($stmtAccStatus->execute()) { returnJsonResponse(true, true, 'Account status updated successfully.'); }
        else { error_log("Change Driver Account Status DB Error: " . $stmtAccStatus->error); returnJsonResponse(false, false, 'Failed to change account status. Database error.'); }
        $stmtAccStatus->close();
    }
    exit;
}
...
// Query and filtering remain the same as previous step (already includes status)
$status_filter = $_GET['status'] ?? ''; // Filters AVAILABILITY
$area_filter = $_GET['area'] ?? '';
$params = []; $types = ""; $main_list_result = null;
...
<head>
    ...
    <style>
        ...
        .action-buttons button { margin-right: 5px; margin-bottom: 5px; /* Allow wrapping */ }
        .action-buttons .availability-btn { background-color: #17a2b8; color: white;} /* Teal */
        .action-buttons .account-status-btn { background-color: #fd7e14; color: white;} /* Orange */
...
<body>
    ...
    <div class="main-content">
        <div class="accounts-header">
            ...
            <div class="filter-section">
                ...
                <select id="statusFilter" onchange="filterDrivers()">...</select>
                <label for="areaFilter">Area:</label>
                <select id="areaFilter" onchange="filterDrivers()">...</select>
            </div>
            <button onclick="openAddDriverForm()" class="add-account-btn">...</button>
        </div>
                ...
                <tbody>
                    ...
                    ?>
                            <tr>
                                ...
                                <td>
                                    <span class="delivery-count <?= $deliveryClass ?>">...</span>
                                    <button class="see-deliveries-btn" onclick="viewDriverDeliveries(...)">...</button>
                                </td>
                                <td class="action-buttons">
                                    ...
                                    <button class="availability-btn" onclick="openAvailabilityModal(<?= $row['id'] ?>, '<?= htmlspecialchars(addslashes($row['name'])) ?>')">
                                        <i class="fas fa-calendar-check"></i> Availability <!-- Updated Icon & Text -->
                                    </button>
                                     <button class="account-status-btn" onclick="openAccountStatusModal(<?= $row['id'] ?>, '<?= htmlspecialchars(addslashes($row['name'])) ?>')">
                                        <i class="fas fa-user-cog"></i> Account <!-- New Button -->
                                    </button>
                                </td>
                            </tr>
                    <?php
    ...
    <div id="addDriverOverlay" class="overlay" style="display: none;">
         <div class="overlay-content">
            ...
            <form id="addDriverForm" method="POST" class="account-form" action="" novalidate>
                ...
                <!-- Fields: Name, Username, Address, Contact, Area, Availability -->
                <label for="add-name">Name:<span class="required-asterisk">*</span></label>
                <input type="text" id="add-name" name="name" required maxlength="40">
                <span id="addNameError" class="error-message"></span>
                <label for="add-username">Username:<span class="required-asterisk">*</span></label>
                <input type="text" id="add-username" name="username" required maxlength="15">
                ...
                <input type="text" id="add-contact_no" name="contact_no" required maxlength="12" pattern="^\d{1,12}$" title="Must be only digits (up to 12)">
                <span id="addContactError" class="error-message"></span>
                <label for="add-area">Area:<span class="required-asterisk">*</span></label>
                <select id="add-area" name="area" required>...</select>
                ...
                <select id="add-availability" name="availability" required>...</select>
                <span id="addAvailabilityError" class="error-message"></span>
                <div class="form-buttons">
                    <button type="button" class="cancel-btn" onclick="closeAddDriverForm()">...</button>
    ...
    <div id="editDriverOverlay" class="overlay" style="display: none;">
        <div class="overlay-content">
            ...
            <form id="editDriverForm" method="POST" class="account-form" action="" novalidate>
                ...
                <!-- Fields: Name, Username, Password, Address, Contact, Area, Availability, Status -->
                 <label for="edit-name">Name:<span class="required-asterisk">*</span></label>
                <input type="text" id="edit-name" name="name" required maxlength="40">
                <span id="editNameError" class="error-message"></span>
                <label for="edit-username">Username:<span class="required-asterisk">*</span></label>
                <input type="text" id="edit-username" name="username" required maxlength="15">
                ...
                <input type="text" id="edit-contact_no" name="contact_no" required maxlength="12" pattern="^\d{1,12}$" title="Must be only digits (up to 12)">
                <span id="editContactError" class="error-message"></span>
                <label for="edit-area">Area:<span class="required-asterisk">*</span></label>
                <select id="edit-area" name="area" required>...</select>
                ...
                <select id="edit-availability" name="availability" required>...</select>
                ...
                <span id="editStatusError" class="error-message"></span>
                <div class="form-buttons">
                    <button type="button" class="cancel-btn" onclick="closeEditDriverForm()">...</button>
                    <button type="submit" class="save-btn">...</button>
                </div>
            </form>
        </div>
    ...
    <div id="accountStatusModal" class="overlay" style="display: none;">
         <div class="overlay-content">
            ...
            <div class="modal-buttons">
                ...
                <button class="archive-btn" onclick="changeAccountStatus('Archive')"> <!-- Changed button class -->
    ...
    <script>
        let currentDriverId = null; // Use a more generic name for the ID being acted upon

        // --- Form Error Handling (clearFormErrors, displayFormErrors remain the same) ---
        ...
        function displayFormErrors(formId, errors) { /* ... same as before ... */ }
        ...
        // --- Deliveries Modal Logic (remains the same) ---
        ...
        function changeAvailability(availability) {
             ...
             $.ajax({
                 ...
                 data: { ajax: true, formType: 'availability_status', id: currentDriverId, availability: availability }, // Send 'availability'
                 dataType: 'json',
                 success: function(response) {
                     ...
                     closeAvailabilityModal(); // Close modal regardless of success/fail
                 },
                 error: function(xhr, status, error) {
                     console.error("Change Availability AJAX Error:", status, error, xhr.responseText);
        ...
        function changeAccountStatus(status) {
             ...
             $.ajax({
                 url: 'drivers.php', type: 'POST',
                 data: { ajax: true, formType: 'account_status', id: currentDriverId, status: status }, // Send 'status'
                 dataType: 'json',
                 success: function(response) {
                     if (response.success && response.reload) {
                         showToast(response.message || 'Account status updated', 'success');
                         setTimeout(() => window.location.reload(), 1500);
                     } else {
                         showToast(response.message || 'Failed to update account status', 'error');
                     }
                     closeAccountStatusModal(); // Close modal regardless of success/fail
                 },
                 error: function(xhr, status, error) {
                     console.error("Change Account Status AJAX Error:", status, error, xhr.responseText);
                     showToast('An error occurred: ' + error, 'error');
        ...
        // --- Client-Side Validation Functions (remain the same) ---
        ...
        $(document).ready(function() {
            ...
            // Add Form Submit (remains the same)
            $('#addDriverForm').on('submit', function(e) { /* ... */ });

            // Edit Form Submit (includes status validation)
            $('#editDriverForm').on('submit', function(e) {
                ...
                isValid &= validateSelect('#edit-status', '#editStatusError', 'Account Status'); // Validate Status Select
