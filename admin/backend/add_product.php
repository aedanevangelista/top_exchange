...
<body>
                ...
                <tbody id="inventory-table">
                    ...
                    if ($result && $result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            ...
                            // *** FIX 3 + DEBUGGING: Apply image display logic here ***
                            echo "<td class='product-image-cell'>";
                            $image_path_db = $row['product_image']; // e.g., /admin/uploads/products/.../image.png
                            $doc_root = $_SERVER['DOCUMENT_ROOT'];
                            $image_path_server = $doc_root . $image_path_db; // e.g., /home/user/public_html/admin/uploads/.../image.png

                            // --- DEBUG LOGGING START ---
                            // Log the paths being used for this specific row/product ID
                            error_log("[Product ID: {$row['product_id']}] Checking image:");
                            error_log("  DB Path: " . $image_path_db);
                            error_log("  Document Root: " . $doc_root);
                            error_log("  Full Server Path for file_exists(): " . $image_path_server);
                            // --- DEBUG LOGGING END ---
                            ...
                            if (!empty($image_path_db) && file_exists($image_path_server)) {
                                ...
                                if (empty($image_path_db)) {
                                    error_log("  Result: No image path in database.");
                                } else {
                                    error_log("  Result: File NOT FOUND at server path.");
                                }
                            }
                            echo "</td>";
    ...
    <script>
        ...
        document.getElementById('add-product-form').addEventListener('submit', function(e) {
            ...
            fetch("../../backend/add_product.php", { // Corrected path
                method: "POST",
                body: formData
            })
        ...
        function editProduct(productId, productType) {
            let apiUrl = `../../backend/get_product.php?id=${productId}`; // Corrected path
                ...
                .then(text => {
                    try {
                        ...
                        if (product.product_image) {
                            // Path already includes /admin/ from backend
        ...
        function viewIngredients(productId, productType) {
            let apiUrl = `../../backend/get_product_ingredients.php?id=${productId}`; // Corrected path
            if (productType === 'walkin') {
                apiUrl += '&type=walkin';
            }
        ...
        function saveIngredients() {
            ...
            if (ingredients.length === 0 && rows.length > 0 && rows.item(0).querySelector('.ingredient-select').value !== '') {
                 // Allow saving empty if the first row wasn't even attempted
            ...
            fetch("../../backend/update_ingredients.php", { // Corrected path
                method: "POST",
        ...
        function updateStock(productId, action, productType) {
            ...
            fetch("../../backend/update_stock.php", { // Corrected path
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                    product_id: productId,
